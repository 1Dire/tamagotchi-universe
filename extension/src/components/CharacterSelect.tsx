...
// 2. 애니메이션별 지속 시간 (ms) - 시간 경과 후 IDLE 복귀용
...
export default function CharacterSelect({ T, onConfirm }: Props) {
  ...
  const handleConfirm = () => {
    ...
    if (typeof chrome !== "undefined" && chrome.storage?.sync) {
      chrome.storage.sync.set({ tamagotchi_data: saveData }, () => {
        onConfirm(c.id);
      });
    } else {
      ...
      <div className="px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => navigate(-1)} 
            disabled={cur === 0}
            className={`p-1 transition-transform ${cur === 0 ? 'opacity-30' : 'hover:scale-110 active:scale-95'}`}
          >
            ◀
          </button>
          ...
          <button 
            ...
            className={`p-1 transition-transform ${cur === CHARACTERS.length - 1 ? 'opacity-30' : 'hover:scale-110 active:scale-95'}`}
          >
            ▶
          </button>
        </div>
